...
const DEFAULT_ITEMS = [
  ...
  "Lighting", "Outlets/Switches", "HVAC", "Plumbing", "Fixtures",
];

const DEFAULT_AREAS = [
  "Unit 1", "Unit 2", "Unit 3", "Unit 4",
  "Lobby / Entrance", "Hallways", "Stairwell", "Laundry Room",
  "Parking Area", "Exterior / Landscaping", "Roof / Gutters", "Storage",
];
...
export default function OnboardingInspectionPage() {
  ...
  function createInspection() {
    ...
    const insp: Inspection = {
      ...
      rooms: DEFAULT_AREAS.map((name) => ({ id: newId(), name, items: DEFAULT_ITEMS.map(blankItem) })),
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Property-level condition documentation for new acquisitions &amp; onboarding</p>
          </div>
              ...
              <tbody>
                {filteredInspections.map((insp) => (
                  <tr key={insp.id} onClick={() => { setActive(insp); setView(insp.status === "completed" ? "completed" : "walking"); }}
                    ...
                    <td className="px-4 py-3 text-muted-foreground">{insp.rooms.length}</td>
  ...
  if (view === "create") {
    return (
      <div className="space-y-6">
        ...
        <p className="text-muted-foreground">Document the condition of an entire property including all units and common areas.</p>
                ...
                <div className="border border-border rounded-lg mt-1 max-h-48 overflow-y-auto bg-card">
                  {filteredProps.slice(0, 20).map((p) => (
                    <button key={p.id} onClick={() => { setForm({ ...form, propertyId: p.id, propertyName: p.name }); setPropSearch(p.name); }}
                      ...
                      {p.name} <span className="text-muted-foreground">--- {p.unitCount} units</span>
                    </button>
                  ))}
                </div>
              )}
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <button onClick={() => { setView("list"); setActive(null); }} className="text-sm text-accent hover:underline">&larr; Back to list</button>
            <h1 className="text-2xl font-bold mt-1">{active.propertyName} --- Onboarding Walk</h1>
            <p className="text-muted-foreground">Document condition of each area across the property</p>
          </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          {active.rooms.map((room, idx) => (
            <button key={room.id} onClick={() => setSelectedRoomIdx(idx)}
              ...
              {room.name}
              {room.items.filter((i) => i.condition !== "").length > 0 && <span className="ml-1 text-xs opacity-70">({room.items.filter((i) => i.condition !== "").length})</span>}
            </button>
          ))}
          <button onClick={() => { const name = prompt("Area name:"); if (name) addRoom(name); }}
            className="shrink-0 px-3 py-1.5 text-sm rounded-lg border border-dashed border-border hover:bg-muted text-muted-foreground">+ Area</button>
        </div>
        ...
        <div className="bg-card rounded-xl border border-border p-4">
          ...
          <textarea value={active.overallNotes} onChange={(e) => save({ ...active, overallNotes: e.target.value, updatedAt: new Date().toISOString() })}
            rows={3} className="w-full text-sm border border-border rounded-lg px-3 py-2 bg-card resize-none" placeholder="General observations about property condition..." />
        </div>
      </div>
    );
  }
