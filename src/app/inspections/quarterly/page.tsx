...
export default function QuarterlyInspectionPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const rows = await fetchInspections("quarterly");
  ...
  // Priority and target date are stored in the notes field as a structured prefix: "[priority|targetDate] actual notes"
  function parseMeta(notes: string) {
    const match = notes.match(/^\[([^|]*)\|([^\]]*)\]\s*([\s\S]*)/);
    if (match) return { priority: match[1], targetDate: match[2], text: match[3] };
    return { priority: "", targetDate: "", text: notes };
  }

  function encodeMeta(priority: string, targetDate: string, text: string) {
    return `[${priority}|${targetDate}] ${text}`;
  }
  ...
  // Count items by priority across active inspection
  function priorityCounts() {
    if (!active) return { low: 0, medium: 0, high: 0, urgent: 0 };
    const allItems = active.rooms.flatMap((r) => r.items);
    const counts = { low: 0, medium: 0, high: 0, urgent: 0 };
    allItems.forEach((item) => {
      const { priority } = parseMeta(item.notes);
      if (priority in counts) counts[priority as keyof typeof counts]++;
    });
    return counts;
  }
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="text-2xl font-bold mt-1">Quarterly Inspections</h1>
            <p className="text-muted-foreground mt-1">Deferred maintenance documentation with priority tracking</p>
          </div>
        ...
        {filteredInspections.length > 0 ? (
          ...
          <div className="text-center py-12 text-muted-foreground">No quarterly inspections yet. Click &quot;+ New Inspection&quot; to begin.</div>
        )}
      </div>
    );
  }
  ...
  if (view === "create") {
    return (
      <div className="space-y-6">
        ...
        <h1 className="text-2xl font-bold">Start Quarterly Inspection</h1>
        <p className="text-muted-foreground">Document deferred maintenance items with priority levels and target completion dates.</p>
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <button onClick={() => { setView("list"); setActive(null); }} className="text-sm text-accent hover:underline">&larr; Back to list</button>
            <h1 className="text-2xl font-bold mt-1">{active.unitNumber} --- Quarterly Inspection</h1>
            <p className="text-muted-foreground">Document maintenance items, assign priority, and set target dates</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          {PRIORITIES.map((p) => (
            <div key={p} className={`rounded-lg border p-3 text-center ${p === "urgent" ? "border-red-300 bg-red-50" : p === "high" ? "border-orange-300 bg-orange-50" : p === "medium" ? "border-yellow-300 bg-yellow-50" : "border-border bg-muted"}`}>
              <p className="text-xs text-muted-foreground capitalize">{p}</p>
              <p className="text-lg font-bold">{counts[p]}</p>
            </div>
          ))}
        </div>
        ...
        {/* Items with priority & target date */}
        {currentRoom && (
          <div className="bg-card rounded-xl border border-border overflow-hidden">
                  ...
                  <div key={item.id} className="p-4 space-y-3">
                    ...
                    <div className="flex gap-3 items-center">
                      <div className="flex-1">
                        ...
                        <select value={meta.priority} onChange={(e) => updateItem(selectedRoomIdx, itemIdx, "notes", encodeMeta(e.target.value, meta.targetDate, meta.text))}
                      ...
                      <div className="flex-1">
                        ...
                        <input type="date" value={meta.targetDate} onChange={(e) => updateItem(selectedRoomIdx, itemIdx, "notes", encodeMeta(meta.priority, e.target.value, meta.text))}
                          className="w-full text-xs border border-border rounded px-2 py-1.5 bg-card" />
                      </div>
                    </div>
                    <input type="text" placeholder="Maintenance notes..." value={meta.text} onChange={(e) => updateItem(selectedRoomIdx, itemIdx, "notes", encodeMeta(meta.priority, meta.targetDate, e.target.value))}
                      className="w-full text-xs border border-border rounded px-2 py-1.5 bg-card" />
                  </div>
                );
              })}
        ...
        <div className="bg-card rounded-xl border border-border p-4">
          ...
          <textarea value={active.overallNotes} onChange={(e) => save({ ...active, overallNotes: e.target.value, updatedAt: new Date().toISOString() })}
            rows={3} className="w-full text-sm border border-border rounded-lg px-3 py-2 bg-card resize-none" placeholder="General deferred maintenance observations..." />
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="text-center py-8">
          ...
          <h1 className="text-2xl font-bold">Quarterly Inspection Complete</h1>
        ...
        <div className="grid md:grid-cols-4 gap-4">
          ...
          <div className="bg-card rounded-xl border border-border p-5 text-center">
            ...
            <p className="text-2xl font-bold mt-1 text-red-600">{counts.urgent + counts.high}</p>
          </div>
          <div className="bg-card rounded-xl border border-border p-5 text-center">
